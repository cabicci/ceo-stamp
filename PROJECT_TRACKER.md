...
| 2026-07-05 | **Bidi POC: per-word layout (fixed)** — rebuild after diagnostic: bidi-js `getVisualWordOrder` + separate `<text>` per word at computed x (no direction attr, text-anchor=start); factors 0.55/0.60, gap 0.30×fontSize; 4 test lines on `/poc-arabic-image`. |
