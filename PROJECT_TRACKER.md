...
| 2026-07-05 | **Bidi diagnostic POC** — `/poc-arabic-image` Tests A/B/C: fixed-x separate `<text>` elements with guide lines (no direction vs direction=rtl vs mixed single text); isolates resvg absolute positioning before next fix attempt. |
