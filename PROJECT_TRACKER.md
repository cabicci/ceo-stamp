...
- **`image_text` on campaign generation** — AI must return a short punchy hook (3–6 words) per `content_item`; `ensureImageText()` guarantees non-null on insert. **Burning wired:** Imagen prompt is always text-free (no contradictory media_brief suffixes; post copy never leaks into image prompt); `burnTextOnImage()` overlays hook when `image_text_language` is `ar` or `en`.
