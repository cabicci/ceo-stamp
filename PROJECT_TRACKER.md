...
- **Lovable preview boot retry** — dev-only root shell retry re-imports TanStack Start's client entry if the preview remains stuck on the SSR boot fallback with no hydrated app content.
...
| 2026-07-02 | **Lovable preview boot retry** — added a dev-only fallback that re-imports the TanStack client entry when the preview stays on the SSR loading shell with empty app content. |
