...
- **`image_text` on campaign generation** — AI must return a short punchy hook (3–6 words) per `content_item`; `ensureImageText()` guarantees non-null on insert. **Burning wired:** Imagen prompt is always text-free; `burnTextOnImage()` overlays hook via resvg + Cairo when `image_text_language` is `ar` or `en`. Burn layout: word-wrap within 80% width, separate `<text>` per line (no overlap), hook truncation guard, vertical centering.
