...
| 2026-07-05 | **Bidi POC: per-word SVG layout** — bidi-js for visual word order only; each word is its own `<text>` at computed x (width estimate + gap); bypasses resvg bidi entirely; 4 test lines on `/poc-arabic-image`; production burn unchanged. |
