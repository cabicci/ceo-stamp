...
const InputSchema = z.object({
  ...
  /** Framework ids from marketing-frameworks.ts (e.g. from approved campaign plan). */
...
export const generateCampaign = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const frameworkIds = resolveFrameworkIds(data.frameworks ?? []);
...
وزّع scheduled_date بشكل متفرّق بين ${data.startDate} و ${data.endDate}. ابدأ توليد JSON دلوقتي.`;
    ...
    // Marketing-science guard: every item must carry framework + substantive rationale.
